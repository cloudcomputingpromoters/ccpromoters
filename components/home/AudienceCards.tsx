  ...
  return (
    <section className="py-16 px-4">
      <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-6">
        {/* Employer card */}
        <div className="bg-[#0B1F3A] rounded-2xl p-10 flex flex-col gap-5 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 group">
          ...
          <ul className="space-y-2">
            {['PE & EIT license verification included', '48-hour candidate turnaround', 'Permanent, contract & executive roles'].map(item => (
              <li key={item} className="flex items-center gap-2 text-white/60 text-sm">
                <div className="w-1.5 h-1.5 bg-[#E91E8C] rounded-full shrink-0" />
                {item}
              </li>
            ))}
          </ul>
          <Link href="/register/employer"
            className="inline-block bg-[#E91E8C] text-white font-semibold px-6 py-3 rounded-lg hover:bg-[#C0176E] transition-colors text-center mt-2">
            Find Talent Now →
          </Link>
        </div>

        {/* Candidate card */}
        <div className="bg-white border-2 border-[#E2E8F0] rounded-2xl p-10 flex flex-col gap-5 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 hover:border-[#E91E8C] group">
          <div className="w-14 h-14 bg-[#E91E8C]/10 rounded-xl flex items-center justify-center">
            <HardHat size={28} className="text-[#E91E8C]" />
          </div>
          <div>
            <h2 className="text-[#0B1F3A] font-bold text-2xl mb-3" style={{ fontFamily: 'Manrope, sans-serif' }}>
