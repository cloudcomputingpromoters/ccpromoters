      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4" style={{ fontFamily: 'Manrope, sans-serif' }}>
            How <span className="text-[#E91E8C]">CCPromoters</span> Works
          </h2>
          {/* Tabs */}
          <div className="inline-flex bg-white/10 rounded-lg p-1 mt-4">
            <button
              onClick={() => setTab('employer')}
              className={`px-6 py-2 rounded-md text-sm font-semibold transition-all ${tab === 'employer' ? 'bg-[#E91E8C] text-white' : 'text-white/70 hover:text-white'}`}>
              For Employers
            </button>
            <button
              onClick={() => setTab('candidate')}
              className={`px-6 py-2 rounded-md text-sm font-semibold transition-all ${tab === 'candidate' ? 'bg-[#E91E8C] text-white' : 'text-white/70 hover:text-white'}`}>
              For Candidates
            </button>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-4 gap-6 mt-8">
          {steps.map((step, i) => (
            <div key={step.n} className="relative">
              {/* Connector line */}
              {i < steps.length - 1 && (
                <div className="hidden md:block absolute top-8 left-1/2 w-full h-0.5 bg-gradient-to-r from-[#E91E8C] to-[#E91E8C]/20 z-0" />
              )}
              <div className="relative bg-white/5 border border-white/10 rounded-xl p-6 hover:bg-white/10 transition-all z-10">
                <div className="w-14 h-14 bg-[#E91E8C] rounded-xl flex items-center justify-center text-white font-bold text-lg mb-4" style={{ fontFamily: 'Space Mono, monospace' }}>
                  {step.n}
                </div>
