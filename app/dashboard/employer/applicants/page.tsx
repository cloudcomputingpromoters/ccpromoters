      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-[#E5E5E5] p-16 text-center">
            ...
            <p className="text-[#6B6B6B] text-sm mb-6">Once candidates apply to your jobs, they will appear here.</p>
          ...
          <div className="space-y-3">
            {jobs.map((job: { id: string; title: string; status: string; applications?: { count: number }[] }) => (
              <Link key={job.id} href={`/dashboard/employer/applicants/${job.id}`}
                className="group flex items-center justify-between bg-white rounded-xl border border-[#E5E5E5] p-5 hover:border-[#CC1016] transition-colors">
                <div>
                  <h3 className="font-bold text-[#0D0D0D] group-hover:text-[#CC1016] transition-colors">{job.title}</h3>
                  <p className="text-sm text-[#6B6B6B] capitalize mt-0.5">{job.status}</p>
                </div>
                <div className="flex items-center gap-2 text-sm font-semibold text-[#CC1016]">
                  <Users size={16} />
