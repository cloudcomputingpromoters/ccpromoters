      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-[#E5E5E5] p-16 text-center">
            <UserCheck size={48} className="text-[#CC1016]/30 mx-auto mb-4" />
            <h3 className="font-bold text-[#0D0D0D] text-xl mb-2">No Saved Candidates</h3>
            <p className="text-[#6B6B6B] text-sm">Candidates you shortlist will appear here for easy reference.</p>
          </div>
        ) : (
              ...
              return (
                <div key={s.id} className="bg-white rounded-xl border border-[#E5E5E5] p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h3 className="font-bold text-[#0D0D0D]">{c?.first_name} {c?.last_name}</h3>
                      {c?.current_title && <p className="text-sm text-[#6B6B6B]">{c.current_title}</p>}
                      <div className="flex flex-wrap gap-3 mt-2 text-xs text-[#6B6B6B]">
                        {(c?.location_city || c?.location_state) && (
                          <span className="flex items-center gap-1"><MapPin size={11} className="text-[#CC1016]" />{[c.location_city, c.location_state].filter(Boolean).join(', ')}</span>
                        )}
                        {c?.discipline && <span className="flex items-center gap-1"><Briefcase size={11} className="text-[#CC1016]" />{c.discipline.replace(/-/g,' ')}</span>}
                        {c?.years_experience != null && <span>{c.years_experience} yrs exp</span>}
                      </div>
                    </div>
                    <button onClick={() => removeSaved(s.id)} className="p-1.5 text-[#6B6B6B] hover:text-red-600 transition-colors shrink-0">
                      <Trash2 size={15} />
                    </button>
                  </div>
                  <p className="text-xs text-[#6B6B6B] mt-3">Saved {new Date(s.saved_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                </div>
              );
            })}
          </div>
