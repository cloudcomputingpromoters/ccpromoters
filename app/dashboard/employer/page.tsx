  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Active Jobs',       value: activeJobs.length,                                      icon: Briefcase, color: '#D4AF37' },
      ...
      {/* CTA */}
      <div className="bg-[#1A3A8F] rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6">
        <div>
        ...
        <Link href="/dashboard/employer/post-job"
          className="bg-[#D4AF37] text-white font-bold px-8 py-3.5 rounded-xl hover:bg-[#B8960C] transition-colors whitespace-nowrap flex items-center gap-2 shrink-0">
      ...
      {jobs.length > 0 ? (
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-6">
          <div className="flex items-center justify-between mb-5">
            <h3 className="font-bold text-[#1A3A8F]">Recent Listings</h3>
            <Link href="/dashboard/employer/listings" className="text-[#D4AF37] text-sm font-semibold hover:underline">
              Manage All →
            </Link>
          </div>
          <div className="space-y-0">
            {jobs.slice(0, 5).map(job => (
              <div key={job.id} className="flex items-center justify-between py-3.5 border-b border-[#E2E8F0] last:border-0 gap-3">
                <div className="flex-1 min-w-0">
                  <Link href={`/jobs/${job.slug}`} target="_blank"
                    className="font-semibold text-[#1A3A8F] hover:text-[#D4AF37] text-sm transition-colors truncate block">
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-12 text-center">
          ...
          <h4 className="font-bold text-[#1A3A8F] mb-2">No listings yet</h4>
          <p className="text-[#4A5568] text-sm mb-5">Post your first civil engineering job to start receiving applications.</p>
          <Link href="/dashboard/employer/post-job"
