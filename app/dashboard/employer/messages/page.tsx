  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5]">
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        {loading ? (
          <div className="h-96 bg-white rounded-xl border border-[#E5E5E5] animate-pulse" />
        ) : threads.length === 0 ? (
          <div className="bg-white rounded-2xl border border-[#E5E5E5] p-16 text-center">
            ...
            <p className="text-[#6B6B6B] text-sm">Candidate conversations will appear here.</p>
          </div>
        ) : (
            ...
            <div className="md:col-span-2 bg-white rounded-2xl border border-[#E5E5E5] overflow-y-auto">
              {threads.map((thread: { id: string; subject?: string; updated_at: string; candidate_name?: string }) => (
                <button key={thread.id} onClick={() => openThread(thread)}
                  ...
                  <p className="font-semibold text-[#0D0D0D] text-sm truncate">{thread.candidate_name || 'Candidate'}</p>
                  <p className="text-xs text-[#6B6B6B] mt-0.5 truncate">{thread.subject || 'Conversation'}</p>
              ...
              {!selectedThread ? (
                ...
                <>
                  <div className="p-4 border-b border-[#E5E5E5]">
                    <p className="font-bold text-[#0D0D0D]">{active?.candidate_name || 'Candidate'}</p>
                    <p className="text-xs text-[#6B6B6B]">{active?.subject || 'Conversation'}</p>
                  </div>
                  <div className="flex-1 overflow-y-auto p-4 space-y-3">
                    {messages.map((msg: { id: string; body: string; sender_role: string; created_at: string }) => (
                      <div key={msg.id} className={`flex ${msg.sender_role === 'employer' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-sm ${msg.sender_role === 'employer' ? 'bg-[#CC1016] text-white' : 'bg-[#F5F5F5] border border-[#E5E5E5] text-[#0D0D0D]'}`}>
                          <p>{msg.body}</p>
                          <p className={`text-xs mt-1 ${msg.sender_role === 'employer' ? 'text-white/70' : 'text-[#6B6B6B]'}`}>
