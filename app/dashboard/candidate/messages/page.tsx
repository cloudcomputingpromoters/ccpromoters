      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-[#E2E8F0] p-16 text-center">
            ...
            <p className="text-[#4A5568] text-sm">When a recruiter reaches out, your conversations will appear here.</p>
          </div>
        ) : (
            ...
            <div className="md:col-span-2 bg-white rounded-2xl border border-[#E2E8F0] overflow-y-auto">
              {threads.map(thread => (
                <button key={thread.id} onClick={() => openThread(thread)}
                  ...
                  <p className="font-semibold text-[#1A3A8F] text-sm truncate">{thread.subject || 'New Message'}</p>
                  <p className="text-xs text-[#4A5568] mt-0.5 truncate">{thread.employer_name || 'CCP Recruiter'}</p>
              ...
              {!selectedThread ? (
                ...
                <>
                  <div className="p-4 border-b border-[#E2E8F0]">
                    <p className="font-bold text-[#1A3A8F]">{active?.subject || 'Conversation'}</p>
                    <p className="text-xs text-[#4A5568]">{active?.employer_name || 'CCP Recruiter'}</p>
                  </div>
                  <div className="flex-1 overflow-y-auto p-4 space-y-3">
                    {messages.map(msg => (
                      <div key={msg.id} className={`flex ${msg.sender_role === 'candidate' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-sm ${msg.sender_role === 'candidate' ? 'bg-[#D4AF37] text-white' : 'bg-[#F7F9FC] border border-[#E2E8F0] text-[#1A3A8F]'}`}>
                          <p>{msg.body}</p>
                          <p className={`text-xs mt-1 ${msg.sender_role === 'candidate' ? 'text-white/70' : 'text-[#4A5568]'}`}>
