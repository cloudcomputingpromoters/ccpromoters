  ...
  return (
    <div className="min-h-screen bg-[#F7F9FC]">
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        ...
        {/* Form */}
        {showForm && (
          <div className="bg-white rounded-2xl border border-[#E2E8F0] p-6 mb-8 space-y-4">
            <h3 className="font-bold text-[#1A3A8F]">{editId ? 'Edit Post' : 'New Post'}</h3>
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold text-[#1A3A8F] mb-1.5">Title</label>
              ...
              <div>
                <label className="block text-sm font-semibold text-[#1A3A8F] mb-1.5">Slug</label>
                <input value={form.slug} onChange={e => setForm({...form, slug: e.target.value})}
                  className="w-full border border-[#E2E8F0] rounded-lg px-4 py-3 text-sm text-[#1A3A8F] focus:outline-none focus:border-[#D4AF37] transition-colors" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-[#1A3A8F] mb-1.5">Category</label>
                <input value={form.category} onChange={e => setForm({...form, category: e.target.value})} placeholder="e.g. Hiring Tips"
                  className="w-full border border-[#E2E8F0] rounded-lg px-4 py-3 text-sm text-[#1A3A8F] focus:outline-none focus:border-[#D4AF37] transition-colors" />
              </div>
            </div>
            <div>
              <label className="block text-sm font-semibold text-[#1A3A8F] mb-1.5">Excerpt</label>
            ...
            <div>
              <label className="block text-sm font-semibold text-[#1A3A8F] mb-1.5">Content (HTML or plain text)</label>
              <textarea value={form.content} onChange={e => setForm({...form, content: e.target.value})} rows={10}
                className="w-full border border-[#E2E8F0] rounded-lg px-4 py-3 text-sm text-[#1A3A8F] focus:outline-none focus:border-[#D4AF37] transition-colors resize-none font-mono text-xs" />
            </div>
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="text-sm text-[#1A3A8F] font-medium">Publish immediately</span>
            </label>
            <div className="flex gap-3">
              ...
              <button onClick={() => { setShowForm(false); setEditId(null); }} className="px-6 py-2.5 border border-[#E2E8F0] rounded-lg text-sm text-[#1A3A8F] hover:border-[#1A3A8F] transition-colors">Cancel</button>
            </div>
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl border border-[#E2E8F0] overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-[#F7F9FC] border-b border-[#E2E8F0]">
                <tr>
                  <th className="text-left px-4 py-3 font-semibold text-[#1A3A8F]">Title</th>
                  <th className="text-left px-4 py-3 font-semibold text-[#1A3A8F] hidden md:table-cell">Category</th>
              ...
              <tbody className="divide-y divide-[#E2E8F0]">
                {posts.map((post: { id: string; title: string; slug: string; category?: string; is_published: boolean; created_at: string }) => (
                  <tr key={post.id} className="hover:bg-[#F7F9FC] transition-colors">
                    <td className="px-4 py-3 font-medium text-[#1A3A8F]">{post.title}</td>
                    <td className="px-4 py-3 text-[#4A5568] hidden md:table-cell">{post.category || '—'}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        {post.is_published && <a href={`/insights/${post.slug}`} target="_blank" rel="noreferrer" className="p-1.5 text-[#4A5568] hover:text-[#D4AF37] transition-colors"><Eye size={15} /></a>}
                        <button onClick={() => startEdit(post)} className="p-1.5 text-[#4A5568] hover:text-[#D4AF37] transition-colors"><Edit3 size={15} /></button>
                        <button onClick={() => deletePost(post.id)} className="p-1.5 text-[#4A5568] hover:text-red-600 transition-colors"><Trash2 size={15} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            {posts.length === 0 && <div className="py-12 text-center text-[#4A5568] text-sm">No blog posts yet.</div>}
          </div>
        )}
      </div>
    </div>
