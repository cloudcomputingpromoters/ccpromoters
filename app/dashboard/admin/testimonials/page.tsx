      ...
      <div className="max-w-5xl mx-auto px-4 py-10 space-y-6">
        ...
        {adding && (
          <form onSubmit={handleAdd} className="bg-white rounded-2xl border border-[#E2E8F0] p-6 space-y-4">
            <h3 className="font-bold text-[#1A3A8F]">New Testimonial</h3>
            ...
            <div>
              <label className="block text-sm font-semibold text-[#1A3A8F] mb-1.5">Quote *</label>
            ...
            <div>
              <label className="block text-sm font-semibold text-[#1A3A8F] mb-1.5">Rating</label>
              <select value={form.rating} onChange={e => setForm({...form, rating: e.target.value})}
                className="border border-[#E2E8F0] rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-[#D4AF37] bg-white">
        ...
        {loading ? (
          <div className="space-y-3">{[1,2,3].map(i => <div key={i} className="h-28 bg-white rounded-xl border border-[#E2E8F0] animate-pulse" />)}</div>
        ) : testimonials.length === 0 ? (
          <div className="bg-white rounded-2xl border border-[#E2E8F0] p-12 text-center text-[#4A5568]">No testimonials yet. Add one above.</div>
        ) : (
          <div className="grid md:grid-cols-2 gap-4">
            {testimonials.map(t => (
              <div key={t.id} className={`bg-white rounded-xl border p-5 ${t.is_published ? 'border-[#E2E8F0]' : 'border-dashed border-[#CBD5E0] opacity-70'}`}>
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div>
                    <p className="font-bold text-[#1A3A8F] text-sm">{t.name}</p>
                    {(t.title || t.company) && <p className="text-xs text-[#4A5568]">{[t.title, t.company].filter(Boolean).join(' · ')}</p>}
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button onClick={() => handleDelete(t.id)} className="p-1.5 text-[#4A5568] hover:text-red-600 transition-colors">
                      <Trash2 size={13} />
                    </button>
                  </div>
                </div>
                <div className="flex gap-0.5 mb-2">
                  {Array.from({ length: t.rating || 5 }).map((_, i) => <Star key={i} size={12} className="text-[#D4AF37] fill-[#D4AF37]" />)}
                </div>
                <p className="text-sm text-[#4A5568] line-clamp-3 italic">&ldquo;{t.quote}&rdquo;</p>
              </div>
            ))}
          </div>
        )}
