...
export default function AdminAnalyticsPage() {
  ...
  const cards = [
    { label: 'Total Jobs', value: stats.totalJobs, sub: `${stats.activeJobs} active`, icon: Briefcase, color: 'text-blue-600 bg-blue-100' },
  ...
  return (
    <div className="min-h-screen bg-[#F7F9FC]">
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 space-y-8">
        {loading ? (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">{[1,2,3,4,5,6].map(i => <div key={i} className="h-28 bg-white rounded-xl border border-[#E2E8F0] animate-pulse" />)}</div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {cards.map(({ label, value, sub, icon: Icon, color }) => (
              <div key={label} className="bg-white rounded-xl border border-[#E2E8F0] p-5">
                ...
                <p className="text-3xl font-bold text-[#1A3A8F]">{value.toLocaleString()}</p>
                <p className="text-sm font-semibold text-[#1A3A8F] mt-0.5">{label}</p>
