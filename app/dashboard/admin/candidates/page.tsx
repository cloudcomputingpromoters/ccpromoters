  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5]">
      ...
      <div className="max-w-6xl mx-auto px-4 py-10">
        ...
        {loading ? (
          <div className="space-y-3">{[1,2,3,4].map(i => <div key={i} className="h-20 bg-white rounded-xl border border-[#E5E5E5] animate-pulse" />)}</div>
        ) : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((c: { id: string; first_name?: string; last_name?: string; current_title?: string; discipline?: string; location_city?: string; location_state?: string; years_experience?: number; created_at: string }) => (
              <div key={c.id} className="bg-white rounded-xl border border-[#E5E5E5] p-4">
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 rounded-full bg-[#0D0D0D] flex items-center justify-center text-white text-sm font-bold shrink-0">
                    {c.first_name?.[0]}{c.last_name?.[0]}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-[#0D0D0D] text-sm">{c.first_name} {c.last_name}</h3>
                    {c.current_title && <p className="text-xs text-[#6B6B6B] truncate">{c.current_title}</p>}
                    <div className="flex flex-wrap gap-2 mt-1.5 text-xs text-[#6B6B6B]">
                      {(c.location_city || c.location_state) && (
                        <span className="flex items-center gap-0.5"><MapPin size={10} className="text-[#CC1016]" />{[c.location_city, c.location_state].filter(Boolean).join(', ')}</span>
                      )}
                      {c.discipline && (
                        <span className="flex items-center gap-0.5 capitalize"><Briefcase size={10} className="text-[#CC1016]" />{c.discipline.replace(/-/g,' ')}</span>
                      )}
                      {c.years_experience != null && <span>{c.years_experience} yrs</span>}
                    </div>
                  </div>
                </div>
                <p className="text-xs text-[#6B6B6B] mt-3">Joined {new Date(c.created_at).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}</p>
              </div>
            ))}
          </div>
        )}
        {!loading && filtered.length === 0 && (
          <div className="py-12 text-center text-[#6B6B6B] text-sm">No candidates found.</div>
        )}
      </div>
    </div>
  );
