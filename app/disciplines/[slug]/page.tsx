...
const disciplineContent: Record<string, {
  ...
  structural: {
    fullName: 'Structural Engineering', tagline: 'Bridges, buildings & heavy infrastructure', color: 'from-rose-600 to-pink-700',
    about: [
  ...
  transportation: {
    fullName: 'Transportation Engineering', tagline: 'Highways, traffic & transit systems', color: 'from-purple-600 to-indigo-700',
    about: [
  ...
  geotechnical: {
    fullName: 'Geotechnical Engineering', tagline: 'Soils, foundations & slope stability', color: 'from-amber-600 to-orange-700',
    about: [
  ...
  'water-resources': {
    fullName: 'Water Resources & Hydrology', tagline: 'Stormwater, hydrology & flood control', color: 'from-cyan-600 to-blue-700',
    about: [
  ...
  environmental: {
    fullName: 'Environmental Engineering', tagline: 'Remediation, air quality & sustainability', color: 'from-green-600 to-emerald-700',
    about: [
  ...
  wastewater: {
    fullName: 'Wastewater & Utilities', tagline: 'Treatment plants, pipelines & utilities', color: 'from-teal-600 to-cyan-700',
    about: [
  ...
  construction: {
    fullName: 'Construction & Project Management', tagline: 'Field engineering & project delivery', color: 'from-orange-600 to-amber-700',
    about: [
  ...
  'land-development': {
    fullName: 'Land Development & Urban Planning', tagline: 'Site civil, entitlements & planning', color: 'from-indigo-600 to-violet-700',
    about: [
  ...
  surveying: {
    fullName: 'Surveying & Geospatial', tagline: 'Land survey, GIS & LiDAR mapping', color: 'from-slate-600 to-gray-700',
    about: [
  ...
  coastal: {
    fullName: 'Coastal & Marine Engineering', tagline: 'Shoreline protection & coastal resilience', color: 'from-blue-600 to-cyan-700',
    about: [
...
export default async function DisciplineDetailPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Hero */}
      <section className={`bg-gradient-to-r ${content.color} relative py-24 px-4 overflow-hidden`}>
        ...
        <div className="relative max-w-5xl mx-auto">
          ...
          <div className="flex flex-wrap gap-3">
            <Link href="/register/candidate" className="bg-white text-[#1A3A8F] font-semibold px-6 py-3 rounded-lg hover:bg-white/90 transition-colors">Submit Your Resume</Link>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-12">
            {/* About */}
            <section>
              <h2 className="text-2xl font-bold text-[#1A3A8F] mb-6" style={{ fontFamily: 'Manrope, sans-serif' }}>About {content.fullName}</h2>
            ...
            {/* Why CCPromoters */}
            <section>
              <h2 className="text-2xl font-bold text-[#1A3A8F] mb-6" style={{ fontFamily: 'Manrope, sans-serif' }}>Why CCPromoters for {content.fullName}?</h2>
              <div className="grid md:grid-cols-3 gap-5">
                {content.differentiators.map((d, i) => (
                  <div key={i} className="bg-[#F7F9FC] rounded-xl p-5 border border-[#E2E8F0]">
                    <div className="w-8 h-8 bg-[#D4AF37] rounded-lg flex items-center justify-center text-white font-bold text-sm mb-3">{i + 1}</div>
                    <h3 className="font-bold text-[#1A3A8F] mb-2">{d.title}</h3>
                    <p className="text-[#4A5568] text-sm">{d.desc}</p>
                  </div>
                ))}
              </div>
            </section>
            ...
            {jobs.length > 0 && (
              <section>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold text-[#1A3A8F]" style={{ fontFamily: 'Manrope, sans-serif' }}>Open Roles in {content.fullName}</h2>
                  <Link href={`/jobs?discipline=${slug}`} className="text-[#D4AF37] font-semibold hover:underline text-sm">View All →</Link>
                </div>
                <div className="space-y-4">
                  {(jobs as Array<{ id: string; title: string; slug: string; employment_type: string; location_city: string; location_state: string; is_remote: boolean; salary_min: number | null; salary_max: number | null; rate_min: number | null; rate_max: number | null; license_required: string | null; is_featured: boolean }>).map(job => (
                    <Link key={job.id} href={`/jobs/${job.slug}`}
                      className="group flex items-start justify-between gap-4 bg-white border border-[#E2E8F0] rounded-xl p-5 hover:border-[#D4AF37] hover:shadow-md transition-all">
                      <div className="flex-1 min-w-0">
                        <h3 className="font-bold text-[#1A3A8F] group-hover:text-[#D4AF37] transition-colors mb-1">{job.title}</h3>
                        <div className="flex flex-wrap gap-3 text-sm text-[#4A5568]">
                          <span className="flex items-center gap-1"><MapPin size={13} className="text-[#D4AF37]" />
                            {job.is_remote ? 'Remote (US)' : `${job.location_city}, ${job.location_state}`}
                          </span>
                          <span className="flex items-center gap-1"><DollarSign size={13} className="text-[#D4AF37]" />{formatSalary(job)}</span>
                          {job.license_required && job.license_required !== 'NONE' && (
                            <span className="flex items-center gap-1"><Award size={13} className="text-[#D4AF37]" />{job.license_required}</span>
                          )}
                        </div>
                      </div>
                      <span className={`shrink-0 text-xs font-bold px-2.5 py-1 rounded-full ${job.employment_type === 'contract' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}`}>
          ...
          <div className="space-y-6">
              ...
              <ul className="space-y-3">
                {content.roles.map((r, i) => (
                  <li key={i} className="flex items-start justify-between gap-2 text-sm pb-3 border-b border-white/10 last:border-0">
                    ...
                    <span className="text-[#D4AF37] font-semibold shrink-0">{r.salary}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="bg-[#D4AF37] rounded-xl p-6 text-white">
              ...
              <Link href="/register/candidate" className="block w-full text-center bg-white text-[#D4AF37] font-bold py-3 rounded-lg hover:bg-white/90 transition-colors">
                Submit Your Resume
              </Link>
            </div>

            {/* Hire talent CTA */}
            <div className="border-2 border-[#1A3A8F] rounded-xl p-6">
              <h3 className="font-bold text-[#1A3A8F] text-lg mb-2">Hiring {content.fullName} Engineers?</h3>
              <p className="text-[#4A5568] text-sm mb-4">We deliver pre-vetted, PE-verified candidates within 48–72 hours of your brief.</p>
              <Link href="/register/employer" className="block w-full text-center bg-[#1A3A8F] text-white font-bold py-3 rounded-lg hover:bg-[#163298] transition-colors">
                Request Talent
              </Link>
            </div>
          </div>
